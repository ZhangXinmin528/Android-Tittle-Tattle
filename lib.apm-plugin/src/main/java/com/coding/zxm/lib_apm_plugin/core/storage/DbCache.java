...
import com.coding.zxm.lib_apm_plugin.core.StorageConfig;
...
public class DbCache {
    ...
    public DbCache(DbHelper db) {
        if (DEBUG) {
            LogX.d(TAG, SUB_TAG, "new DbCache");
        }
    ...
    public boolean saveDataToDB(InfoHolder data) {
        if (data == null) {
            if (DEBUG) {
                LogX.d(TAG, SUB_TAG, "saveDataToDB To tmp list data == null");
            }
            return false;
        }
                ...
                switch (msg.what) {
                    case MSG_WHAT_PRE_WRITE_DB:
                        ...
                        try {
                            ...
                            LogX.d(TAG, SUB_TAG, "class cast exception : " + ex.getMessage());
                        }
                        if (holder != null) {
                            handleData(holder);
                        } else {
                            if (DEBUG) {
                                LogX.d(TAG, SUB_TAG, "holder == null");
                            }
                        }
                        break;
                    case MSG_WHAT_WRITE_DB:
                        if (DEBUG) {
                            LogX.d(TAG, SUB_TAG, "MSG_WHAT_WRITE_DB");
                        }
                    ...
                    case MSG_WHAT_TIME_OUT:
                        if (DEBUG) {
                            LogX.d(TAG, SUB_TAG, "MSG_WHAT_TIME_OUT -> MSG_WHAT_WRITE_DB");
                        }
    ...
    private void handleData(InfoHolder data) {
        ...
        if (DEBUG) {
            LogX.d(TAG, SUB_TAG, "saveDataToDB size = " + size + " interval = " + interval + " : name " + data.tableName + " | info : " + data.info.toString());
        }
        ...
        if (interval >= StorageConfig.SAVE_DB_INTERVAL || size >= StorageConfig.SAVE_DB_MAX_COUNT) {
            if (DEBUG) {
                LogX.d(TAG, SUB_TAG, "saveDataToDB To db size = " + size);
            }
    ...
    private void updateTime(long cur) {
        if (DEBUG) {
            LogX.d(TAG, SUB_TAG, "updateTime = " + cur);
        }
        mLastTime = cur;
    }
    ...
    private void readFromListAndWriteToDB() {
        if (DEBUG) {
            LogX.d(TAG, SUB_TAG, "readFromListAndWriteToDB: tid = " + Thread.currentThread().getId() + " start");
        }

        if (myDataList == null || myDataList.isEmpty()) {
            if (DEBUG) {
                LogX.d(TAG, SUB_TAG, "readFromListAndWriteToDB: myDataList empty");
            }
            return;
        }
        ...
        if (db == null) {
            if (DEBUG) {
                LogX.d(TAG, SUB_TAG, "readFromListAndWriteToDB: db == null");
            }
            return;
        }
        int count = 0;
