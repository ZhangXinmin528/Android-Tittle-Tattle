...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public DbHelper(Context context, boolean isInSdcard) {
        ...
        if (DEBUG) {
            LogX.d(TAG, SUB_TAG, "db isInSdcard = " + isInSdcard);
        }
    ...
    public SQLiteDatabase getDatabase() {
        if (mDb == null) {
            try {
                ...
                if (DEBUG) {
                    LogX.e(TAG, SUB_TAG, "getDatabase ex : " + Log.getStackTraceString(e));
                }
            }
        }
        return mDb;
    ...
    private void newDbFile(String path) {
        if (DEBUG) {
            LogX.d(TAG, SUB_TAG, "db path = " + path);
        }
        ...
        try {
            ...
            if (DEBUG) {
                LogX.d(TAG, SUB_TAG, "newDbFile ioException : " + Log.getStackTraceString(e));
            }
        }
    }
