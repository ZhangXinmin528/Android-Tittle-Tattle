package com.coding.zxm.lib_apm_plugin.core.job.fileinfo;

import android.content.ContentValues;

import com.coding.zxm.lib_apm_plugin.core.BaseInfo;
...
public class FileInfo extends BaseInfo {
    ...
    public void parserJson(JSONObject json) throws JSONException {
        this.mFileName = json.getString(DBKey.KEY_FILE_NAME);
        this.mFilePath = json.getString(DBKey.KEY_FILE_PATH);
        this.mFileType = json.getInt(DBKey.KEY_FILE_TYPE);
        this.mLastModified = json.getLong(DBKey.KEY_LAST_MODIFIED);
        this.mFileSize = json.getLong(DBKey.KEY_FILE_SIZE);
        this.mWritable = json.getInt(DBKey.KEY_WRITABLE);
        this.mReadable = json.getInt(DBKey.KEY_READABLE);
        this.mExecutable = json.getInt(DBKey.KEY_EXECUTABLE);
        this.mSubFIleNum = json.getLong(DBKey.KEY_SUB_FILE_NUM);
    }
    ...
    public ContentValues toContentValues() {
        ...
        try {
            values.put(DBKey.KEY_FILE_NAME, mFileName);
            values.put(DBKey.KEY_FILE_PATH, mFilePath);
            values.put(DBKey.KEY_FILE_TYPE, mFileType);
            values.put(DBKey.KEY_LAST_MODIFIED, mLastModified);
            values.put(DBKey.KEY_FILE_SIZE, mFileSize);
            values.put(DBKey.KEY_WRITABLE, mWritable);
            values.put(DBKey.KEY_READABLE, mReadable);
            values.put(DBKey.KEY_EXECUTABLE, mExecutable);
            values.put(DBKey.KEY_SUB_FILE_NUM, mSubFIleNum);
        } catch (Exception e) {
            e.printStackTrace();
        }
