package com.coding.zxm.lib_queue.polling.util;
 ...
 * android log utils
 */
public final class TaskDebuger {
     ...
     * @param enable  {@link TaskDebuger.LogConfig#setLogSwitch(boolean)}
     */
    public static void setLogEnable(@NonNull Context context, @NonNull boolean enable) {

        if (context != null) {
            //log
            final TaskDebuger.LogConfig logConfig =
                    new TaskDebuger.LogConfig(context)
                            .setLogSwitch(enable);
            TaskDebuger.resetLogConfig(logConfig);

            TaskDebuger.d(logConfig.toString());
        }
